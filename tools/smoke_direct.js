...
console.log('\n[D9] 직영작업현황 — 기본 오늘 · 기간으로 조회 (v2.19.11 사용자 지시)');
{
  ...
  /* ★스탭으로 본다 (v2.49.1) — 이 검사는 **탭7의 직영작업현황 카드**가 대상이다.
     관리자에게는 탭7이 감춰져 A.go(7)이 작업현황(탭1)으로 떨어지는데, 거기서는
     직영이 「협력업체, 직영 작업현황」 카드 안에 **제목 없이** 들어간다(사용자
     지시 「제목이 몇 개야」). 그래서 탭1에서 카드 이름을 찾으면 안 나온다. */
  A.setRole('staff'); S.lang = 'ko';
