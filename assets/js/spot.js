...
(function (G) {
  ...
  /* ★측점은 **고르는 것이다** — 직접입력 금지(사용자 확정).
     ...
     ★그런데 **번호(앞 칸)** 는 손으로 쳐도 된다 — 어떤 정수든 유효한
       측점이라 없는 측점이 생길 수 없다. 10km 도로면 No.500까지 가는데
       드롭다운으로는 못 고른다(사용자 지적). 그래서 번호는 숫자 입력칸으로
       바꿨다(vendor.js staSel). 「직접입력 금지」는 **뒤 +칸(0~19)에만**
       적용된다 — 거기가 1+20을 만드는 자리다. staNos/STA_NO_MAX(번호 드롭다운
       목록·상한)는 그래서 지웠다. */
